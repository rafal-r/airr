/* global HTMLElement, Airr, MVCObject, View */
...
Scene.prototype.enableMayer = function (contentObj, buttons, attrs) {
    ...
    if (contentObj instanceof HTMLElement) {
        text.appendChild(contentObj);
    } else if (contentObj instanceof MVCObject) {
        text.appendChild(contentObj.dom);

        if (contentObj.mayerButtons && (typeof buttons === 'undefined' || buttons.length === 0)) {
            buttons = contentObj.mayerButtons;
        }
    } else if (typeof contentObj === "string") {
        text.innerHTML = contentObj;
    } else {
        console.error(this.name + '::enableMayer ivalid content argument');
    }
    ...
    if (buttons && buttons.length) {
        ...
        for (var i = 0; i < buttons.length; i++) {
            ...
            if (buttons[i].text) {
                if (typeof buttons[i].text === 'string') {
                    btn.innerText = buttons[i].text;
                } else if (typeof buttons[i].text === 'function') {
                    btn.innerText = buttons[i].text();
                }
            }
            if (buttons[i].style) {
            ...
            if (buttons[i].handler) {
                if (contentObj instanceof MVCObject) {
                    btn.addEventListener('click', (function (handler) {
                        return function (e) {
                            handler.call(contentObj, e);
                        };
                    })(buttons[i].handler));
                } else {
    ...
    if (contentObj instanceof MVCObject) {
        contentObj.triggerWhenRenderedQueue();
    }

    //if mayer is bigger then screen height
